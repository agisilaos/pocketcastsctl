  ...
  build_one() {
    ...
    GOOS=darwin GOARCH="${goarch}" CGO_ENABLED=0 \
      go build -trimpath \
        ...
        -o "${stage}/pocketcastsctl" \
        ./cmd/pocketcastsctl

    (cd "${stage}" && tar -czf "../pocketcastsctl_${ver}_darwin_${goarch}.tar.gz" pocketcastsctl)
    rm -rf "${stage}"
  }
...
update_homebrew_formula() {
  ...
  sha_amd="$(awk -v f="pocketcastsctl_${ver}_darwin_amd64.tar.gz" '$2==f{print $1}' dist/SHA256SUMS.txt | head -n 1 || true)"
...
class Pocketcastsctl < Formula
  ...
  on_macos do
    on_arm do
      url "https://github.com/${repo_owner}/${repo_name}/releases/download/${ver}/pocketcastsctl_${ver}_darwin_arm64.tar.gz"
      sha256 "${sha_arm}"
    end
    on_intel do
      url "https://github.com/${repo_owner}/${repo_name}/releases/download/${ver}/pocketcastsctl_${ver}_darwin_amd64.tar.gz"
      sha256 "${sha_amd}"
    end
  end
...
if "pocketcastsctl" not in formulae:
    formulae.append("pocketcastsctl")
    formulae.sort()
...
PY
  ...
  git add Formula/pocketcastsctl.rb README.md
  if git diff --cached --quiet; then
    print -- "Homebrew formula already up to date"
    popd >/dev/null
    return 0
  fi
  git commit -m "pocketcastsctl ${ver}"
