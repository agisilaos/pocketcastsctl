...
CLI_NAME="pocketcastsctl"
FORMULA_NAME="pocketcastsctl"
ARTIFACT_NAME="pocketcastsctl"
DEFAULT_BRANCH="main"
DEFAULT_HOMEBREW_DESC="macOS CLI for Pocket Casts Web Player control"
DEFAULT_HOMEBREW_LICENSE="MIT"
DEFAULT_HOMEBREW_TEST_ARG="version"
DEFAULT_FORMULA_PATH="Formula/pocketcastsctl.rb"
DEFAULT_BUILD_PKG=""
...
if [[ "$current_branch" != "$DEFAULT_BRANCH" ]]; then
  if [[ "$DRY_RUN" -eq 1 ]]; then
    ...
    err "release must run from $DEFAULT_BRANCH (current: ${current_branch:-detached HEAD})"
  fi
fi
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  ...
  render_formula "$dry_formula_path"
  validate_formula "$dry_formula_path"
